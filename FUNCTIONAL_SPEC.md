...
- l assistant IFC4 ne cree pas directement de noeuds spatiaux ni d equipements : l utilisateur doit encore passer par les etapes imports standard `preview`, `validate` et `execute`
...
- l assistant IFC4 permet maintenant de mapper explicitement les proprietes `IfcPropertySingleValue` vers les champs equipement `code equipement`, `num piece`, `reference externe`, `categorie`, `famille`, `sous-famille`, `type`, `marque`, `modele`, `statut` et `proprietaire`, avec recalcul volontaire via le bouton `Actualiser le mapping`
...
- `apps/web`
  ...
  - module `imports` avec diagnostic de geometrie IFC obligatoire, badges `Geometrie OK`, `Geometrie manquante` et `Erreur geometrie`, et blocage des jobs si une ligne selectionnee n est pas exploitable
